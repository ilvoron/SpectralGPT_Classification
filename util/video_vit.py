...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        assert (H == self.img_size[0] and W == self.img_size[1]), \
            f"Размер входного изображения ({H}*{W}) не совпадает с моделью ({self.img_size[0]}*{self.img_size[1]})."
...
class Attention(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert dim % num_heads == 0, "dim должна делиться на num_heads"
...
class Block(nn.Module):
    """
    Трансформер-блок с заданной функцией внимания
    """
    ...
    def __init__(
        ...
        # Drop path для стохастической глубины: смотрим, лучше ли это, чем dropout здесь
